...
namespace CoolCompilator
{
    ...
    public class AstNodeCodeGenerator : AstNodeVisitor
    {
        ...
        Dictionary<Symbol, Dictionary<Symbol, int>> method_tbl=new Dictionary<Symbol, Dictionary<Symbol, int>>();

        Dictionary<Symbol, Dictionary<Symbol, int>> attr_tbl=new Dictionary<Symbol, Dictionary<Symbol, int>>();
        ...
        public void code_dispatch_table(Class class_node)
        {
            ...
            while (recur.Count != 0)
            {
                ...
                foreach (var method in head.methods)
                {
                    ...
                    {
                        if (!method_tbl.ContainsKey(class_node.name))
                            method_tbl.Add(class_node.name, new Dictionary<Symbol, int>());
                        dispoffset += 1;
                        method_tbl[class_node.name].Add(method.name, dispoffset);
        ...
        public override void visit(Attribute attr)
        {
            ...
            ++curr_attr_count;
            if (!attr_tbl.ContainsKey(curr_class))
                attr_tbl.Add(curr_class, new Dictionary<Symbol, int>());
            attr_tbl[curr_class].Add(attr.name, curr_attr_count);
